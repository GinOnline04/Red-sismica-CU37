...
public class GestorCerrarOrdenInspeccion {
    ...
    public void tomarConfirmacionCierreOrden(boolean confirmacion){
        ...
        if (confirmacion){
        if(validarExistenciaObservacion(observacionCierreOrdenInspeccion) && validarExistenciaMotivoSeleccionado(motivosTipoSeleccionados) && validarExistenciaComentario(comentarios) ){
    ...
    public boolean validarExistenciaObservacion(String observacion) {
        if (observacion == null || observacion.trim().isEmpty()) {
            interfaz.mostrarMensajeError("Error: Debe ingresar Comentarios, cancelando cierre de orden");
    ...
    public boolean validarExistenciaComentario(List<String> comentariosIngresados) {
        if (comentariosIngresados == null || comentariosIngresados.isEmpty()) {
            interfaz.mostrarMensajeError("Error: Debe ingresar Comentarios, cancelando cierre de orden");
            System.out.println("Debe ingresar Comentarios.");
            return false;
        }
        for (String comentario : comentariosIngresados) {
            if (comentario == null || comentario.trim().isEmpty()) {
                interfaz.mostrarMensajeError("Error: Hay comentarios vacíos o nulos, cancelando cierre de orden");
                System.out.println("Hay comentarios vacíos o nulos.");
                return false;
            }
        }
        return true;
    ...
    public boolean validarExistenciaMotivoSeleccionado(List<MotivoTipo> motivosSeleccionados) {
        if (motivosSeleccionados == null || motivosSeleccionados.isEmpty()) {
            interfaz.mostrarMensajeError("Error: Debe seleccionar al menos un motivo de fuera de servicio, cancelando cierre de orden");
    ...
    public void generardatos() {
        ...
        // ROLES
        // ---------
        Rol responsableInspeccion= new Rol("ResponsableInspeccion", "Encargado de coordinar y realizar reparaciones del sismógrafo.");
        ...
        // EMPLEADOS
        // ---------
        Empleado emp1 = new Empleado("Lucía", "Gómez", "lucia.gomez@sismo.gob.ar", "3511234567", responsableInspeccion);
        Empleado emp2 = new Empleado("Martín", "Fernández", "martin.fernandez@sismo.gob.ar", "3512345678", responsableInspeccion);
        Empleado emp3 = new Empleado("Sofía", "López", "sofia.lopez@sismo.gob.ar", "3513456789", responsableInspeccion);
    ...
    public void cancelar(){
            System.out.println("Cancelacion de Cerrar Orden");
            System.exit(1);
    }

    public void finCu(){
        System.out.println("Operacion Finalizada con exito");
        interfaz.mostrarMensaje("Operacion Finalizada con exito");
        System.exit(0);
    }
}
